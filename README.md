...
```В дальнейшем, конечно, можно добавить оптимизации и к другим частовстречаемым операциям
...
| `sort` | [`golden/sort.yml](golden/sort.yml) | Алгоритм получает на вход число $N$ и далее читает $N$ чисел из числового буфера ввода. Для хранения массива использовалась `allot` по аналогии с `hello user name`. Выделяется буфер для массива в 64 слова. В рамках философии `pstr` в этом алгоритме массив хранится как `N, arr[0], ..., arr[n-1]` - похожим образом на `pstr`. Алгоритм демонстрирует работу с массивом: итерацию по значениям, операции с элементами (сравнения, перестановки). В конце печатает получившийся отсортированный массив (**Bubble Sort**, $O(N^2)$) через `, ` | 24954 для $N=10$ |
